...
class loanQueryHandler {
    ...
    async getOneInstallment(loan_id, cuote_number) {
        return await this.db.getData('instalments', `loan_id = ${loan_id} and instalment_number = ${cuote_number}`)
    }
    ...
    async firstConfirmation(newStatus) {
        const loan = await this.db.getOne('loans', `id = ${this.source.loanId}`, 'id, amount, debtor_id')
        const instalmentId = await this.db.getOne('instalments', `instalment_number = 0 AND loan_id = ${loan.id}`)
    ...
    async secondConfirmation(newStatus) {
        const transaction = await this.db.getOne('instalments', `loan_id = ${this.source.loanId} AND instalment_number = 0`, 'transaction_id')
