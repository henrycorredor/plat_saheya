...
class loanQueryHandler {
    ...
    async treasurerConfirmDisbursement() {
        await this.db.upsert('loans', { status: 6, last_update: moment().format("YYYY-MM-DD hh:mm:ss") }, `id = ${this.source.loanId}`)
    }

    async userConfirmDisbursement() {
        await this.db.upsert('loans', { status: 7, last_update: moment().format("YYYY-MM-DD hh:mm:ss") }, `id = ${this.source.loanId}`)
    }

    async bothSidesConfirm() {
        await this.db.upsert('loans', { status: 8, last_update: moment().format("YYYY-MM-DD hh:mm:ss") }, `id = ${this.source.loanId}`)
    }
    ...
    async treasurerDisbursement() {
        ...
        // register money given to user as a payment
