...
class PaymentsQueryHandler {
    ...
    async aprovePayment(payment_id, user_id) {
        ...
        if (loanPayment) {
            ...
            let totalPayment = loanPayment.instalment
            const dataArray = []
            cosigners.map(cosigner => {
                const onDebt = cosigner.guaranteed_payed
                const guaranteed = cosigner.guaranteed_amount
                let myPayment = onDebt - totalPayment
                if (totalPayment > 0) {
                    myPayment = (myPayment<0) ? totalPayment : onDebt
                }else{
                    myPayment = (myPayment > guaranteed) ? guaranteed : totalPayment
                }
            ...
            const setCosignerPasive = dataArray.map(async cosigner => {
                ...
                await this.db.upsert('cosigner_rels', { guaranteed_payed: cosigner.payed + cosigner.myPayment }, `loan_id = ${loanPayment.loan_id} and cosigner_id = ${cosigner.id}`)
            })
