...
const cosignersHandler = require('../lib/cosigners_handler')
...
const conditions = {
    ...
    selfDebtMaxAmount: async function ([capital_source, schema_percent, all_amount_selfsuported], user_loan_amount) {
        if (!all_amount_selfsuported) {
            const freeCapitalVsLoanPercent = await this.getCapitalVsLoanPercent(capital_source, user_loan_amount, process.env.USER_ID)
            return this.compare(schema_percent, freeCapitalVsLoanPercent, `El monto supera por ${Math.floor(freeCapitalVsLoanPercent)}% la cantidad permitida para este tipo de préstamo.`)
        } else {
            return [true, 'pass']
        }
    },
...
const val = {
    validator: async function (user_loan_aplication) {
        ...
        if (this.filters.selfDebtMaxAmount[2]) {
